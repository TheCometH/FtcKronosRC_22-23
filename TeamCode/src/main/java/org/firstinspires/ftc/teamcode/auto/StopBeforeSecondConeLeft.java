...
@Autonomous(name = "StopBeforeSecondConeLeft")
public class StopBeforeSecondConeLeft extends LinearOpMode {
...
//                .build();
        ...
        if(!isStopRequested()) {
            ...
            while (isStarted() && !isStopRequested() && idFound && (System.currentTimeMillis() - start) < 6000)
            {
                try {
                    camera.detect();
